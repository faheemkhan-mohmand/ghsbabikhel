        ...
        {mockLibrary.map(item => (
          <div key={item.id} className="card-matte p-5 hover:shadow-lift transition-shadow">
            <div className="flex items-start gap-3">
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${item.fileType === 'pdf' ? 'bg-destructive/10' : 'bg-primary/10'}`}>
                {item.fileType === 'pdf' ? <FileText className="w-4 h-4 text-destructive" /> : <FileIcon className="w-4 h-4 text-primary" />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <Button size="sm" variant="outline" className="btn-press gap-1 text-xs mt-3">
